...
Register-ArgumentCompleter -CommandName Set-GitDirectoryLocation,cdg -ParameterName Path -ScriptBlock {
    ...
    $paths | ForEach-Object {
        [System.Management.Automation.CompletionResult]::new(
            ($_.FullName -replace [regex]::Escape($env:LOCAL_GIT_FOLDER),'').Trim('\'),  # completion text - what's sent to the command
            ($_.FullName -replace [regex]::Escape($env:LOCAL_GIT_FOLDER),'').Trim('\'),               # list item text - what's displayed in the list
